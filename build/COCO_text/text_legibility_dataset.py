...
class TextLegibility_TrueOrFlase(Dataset):
    # the TrueOrFlase version of the visual Dialog dataset
    def __init__(self, args, config='datasets/configs/TextLegibility_val.yaml', proc=None, duplication=1):
        if type(config) == str:
        ...
        self.args = args
        if args.core_eval:
            data = json.load(open(self.config['data_config']['core_true_or_false_path'], 'r'))
        else:
...
if __name__=='__main__':
    ds = TextLegibility_TrueOrFlase(args='')
