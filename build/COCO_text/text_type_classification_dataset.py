...
def crop_bbox(img, bbox):
    W,H = img.size
    x, y, w, h = bbox
    x1, y1, x2, y2 = int(x), int(y), np.minimum(int(x + w),W), np.minimum(int(y + h),H)
...
class TextTypeClassification_SingleChoice(Dataset):
    # the TrueOrFlase version of the visual Dialog dataset
    def __init__(self, args, config='datasets/configs/TextTypeClassification_val.yaml', proc=None, duplication=1):
        if type(config) == str:
        ...
        self.args = args
        if args.core_eval:
            data = json.load(open(self.config['data_config']['core_multiple_choice_path'], 'r'))
        else:
            data = json.load(open(self.config['data_config']['multiple_choice_path'], 'r'))
        # data = json.load(open(self.config['data_config']['multiple_choice_path'], 'r'))
    ...
    def __getitem__(self, index):
        ...
        if self.duplication > 1:
            ...
            new_sample['question'] = self.samples[sample_index]['question'][inner_sample_index % len(self.samples[sample_index]['question'])]
        else:
            # randomly choose one prompt
            new_sample['instruct'] = random.choice(self.instruction_list)
            new_sample['question'] = random.choice(self.samples[sample_index]['question'])
        ...
        image = crop_bbox(raw_image, new_sample['bbox'])
...
if __name__=='__main__':
    ds = TextTypeClassification_SingleChoice(args='')
