...
# Caption for Flickr30K
class Caption(Dataset):
    def __init__(self, args, config='datasets/configs/Caption_Flickr30K_val.yaml', proc=None, duplication=1):
        if type(config) == str:
        ...
        if args.hf:
            ...
            data = json.load(open(self.config['data_config']['flickr30k_path'], 'r'))
        assert data['dataset_name'] == 'flickr30k'
        data = data['data']
