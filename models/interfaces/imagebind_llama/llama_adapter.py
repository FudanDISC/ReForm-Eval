...
class LLaMA_adapter(nn.Module):
    ...
    def __init__(self, llama_ckpt_dir, llama_tokenizer, knn=False, phase="finetune", legacy_bridge=False):
        ...
        model_args: ModelArgs = ModelArgs(
            max_seq_len=1400, max_batch_size=1, w_bias=bias_lora, w_lora=bias_lora,  **params
        ...
        if knn:
            ...
            self.index = faiss.read_index("/remote-home/share/multimodal-models/LLaMA-Adapter/ckpts_imagebindllm/knn.index")
    ...
    def set_default_trainability(self, phase='finetune'):
        ...
        print('all',sum([param.numel() for name, param in self.named_parameters()]))
        for key, value in self.get_trainable_params(phase).items():
            value.data = value.data.float()
            value.requires_grad = True
        print('trainable',sum([param.numel() for name, param in self.get_trainable_params(phase).items()]))
        print(phase)
...
def load(model_path, llama_dir, device="cuda" if torch.cuda.is_available() else "cpu", download_root='ckpts',
